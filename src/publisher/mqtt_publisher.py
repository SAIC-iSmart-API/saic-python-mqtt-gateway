...
import logging
import ssl
from typing import TYPE_CHECKING, Any, Final, cast, override, Optional
...
class MqttPublisher(Publisher):
    ...
    async def __run_loop(self) -> None:
        ...
        while True:
            try:
                ...
                LOG.error(
                    f"Connection to MQTT broker lost; Reconnecting in {interval} seconds ...",
                    exc_info=True,
                )
    ...
    async def __enable_commands(self) -> None:
        ...
        try:
            ...
            LOG.error("Failed to subscribe to MQTT command topics", exc_info=e)
            raise e
